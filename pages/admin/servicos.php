...
if(isset($_POST['save'])){
    ...
    if(isset($_POST['id']) && $_POST['id'] != ''){
        switch($_POST['action']){
            case 'update':
                $stmt = $conexao->prepare("UPDATE servicos SET nome = :nome, descricao = :descricao WHERE id = :id ");
        ...
        $stmt = $conexao->prepare("INSERT INTO servicos (nome, descricao) VALUES (:nome, :descricao)");
        $stmt->bindValue('nome', $nome, PDO::PARAM_STR);
        $stmt->bindValue('descricao', $descricao, PDO::PARAM_STR);
        try{
            $stmt->execute();
...
$stmt = $conexao->prepare("SELECT * FROM servicos ORDER BY id DESC");
$stmt->execute();
